...
def generate_data(fct, x, time_steps, seperate=False):
    ...
    conn = MySQLdb.connect(host='localhost', user='root', passwd='111111', db='mydb', port=3306)

    df = pd.read_sql('SELECT sum(power_consumption) FROM mydb.Tianchi_power group by record_date order by record_date limit 600',
                     conn)
